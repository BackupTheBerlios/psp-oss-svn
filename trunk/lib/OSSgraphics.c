...
// Load a Graphic from RAR into Memory

char *hardrarfile;
static int rar_activate = 0;
char *rardata_ptr;
int rardata_ptr_offset = 0;
...
void user_read_data_fn(png_structp png_ptr, png_bytep data, png_size_t length) {
	...
	if (rar_activate == 1) {
		...
		rardata_ptr = NULL;
		rardata_ptr_offset = 0;
		urarlib_get(&rardata_ptr, &data_size, hardrarfile, (png_FILE_p)png_ptr->io_ptr, "ejmXnC88");
		rar_activate = 0;

	}

	png_memcpy(data, (const char*)rardata_ptr + rardata_ptr_offset, length);

	rardata_ptr_offset += length;
	
}


Image* LoadGFX_RAR(const char* filename, const char* rarfile)
{
        png_structp png_ptr;
        png_infop info_ptr;
        unsigned int sig_read = 0;
	...
	png_set_read_fn(png_ptr, (png_voidp) NULL, user_read_data_fn);
        png_set_error_fn(png_ptr, (png_voidp) NULL, (png_error_ptr) NULL, user_warning_fn);
        info_ptr = png_create_info_struct(png_ptr);
        if (info_ptr == NULL) {
                free(image);
